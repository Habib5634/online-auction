...
const ProfileMenu = () => {
  ...
  return (
    <div className=''>
      ...
      <div className='h-5 w-5 rounded-full text-white bg-red absolute -top-3 -right-3 flex justify-center items-center'>{notifications?.length}</div>
      </div>
      <div className={`${openMenu ? 'min-h-full p-3 shadow-shad':'h-0 overflow-hidden'}  absolute right-0 rounded-lg flex flex-col gap-2 items-start bg-white w-full max-w-[150px] anim3`}>
       <Link href={'/profile'}> <button className='font-semibold '>Profile</button></Link>
        <button className='font-semibold '>Notification <span className='text-red font-semibold'>{notifications?.length}</span>{}</button>
